...
func (h *Handler) obtainLoginBonus(tx *sqlx.Tx, userID int64, requestAt int64) ([]*UserLoginBonus, error) {
	...
	for _, bonus := range loginBonuses {
		...
		// 追加アイテムを保存
		addItems = append(addItems, AddItem{ItemID: rewardItem.ItemID, ItemType: rewardItem.ItemType, Amount: rewardItem.Amount})
...
func (h *Handler) obtainPresent(tx *sqlx.Tx, userID int64, requestAt int64, c echo.Context) ([]*UserPresent, error) {
	...
	receivedArray := make([]*UserPresentAllReceivedHistory, 0)
